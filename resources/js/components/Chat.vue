...
onMounted(() => {
    ...
    // Listen to the current user's channel to receive messages sent to them
    console.log('Setting up Echo listener for user:', auth.user.id);
    
    echoChannel = window.Echo.private(`chat.${auth.user.id}`)
        .listen('MessageSent', (e) => {
            console.log('Message received via Echo:', e);
            console.log('Message sender ID:', e.message.sender.id);
...
// Clean up Echo listener when component is unmounted
onUnmounted(() => {
    if (echoChannel) {
        window.Echo.leaveChannel(`chat.${auth.user.id}`);
    }
});
...
const sendMessage = () => {
    ...
    router.post(route('chat.send'), payload, {
        preserveScroll: true,
        onSuccess: (page) => {
